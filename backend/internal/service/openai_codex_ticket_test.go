		...
		responses: []*http.Response{
			{
				...
				Header:     header312,
				Body:       io.NopCloser(strings.NewReader(ticketProbeSSE("gpt-6-astra"))),
			},
			{
				StatusCode: http.StatusOK,
				Header:     header292,
...
func (u *codexTicketConcurrentUpstream) Do(req *http.Request, _ string, _ int64, _ int) (*http.Response, error) {
	...
	return &http.Response{StatusCode: 200, Header: h, Body: io.NopCloser(strings.NewReader(ticketProbeSSE(request.Model)))}, nil
}
func TestRefreshOpenAICodexTickets_ConcurrentModelsPreserveAccountSnapshot(t *testing.T) {
	...
	require.Len(t, repo.updates, 4) // tickets plus persistent generation markers
