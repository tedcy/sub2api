...
// ErrOpenAICodexTicketUnavailable 表示该号该模型没有可用的 Codex 门票，
// 且 fail_closed 禁止裸打业务请求。
var ErrOpenAICodexTicketUnavailable = errors.New("codex turn-state ticket unavailable")

var errOpenAICodexTicketModelDisabled = errors.New("ticket model is not selected")
...
// Team uses 332-byte tickets; personal/unknown plans retain the configured target.
...
func (s *OpenAIGatewayService) openAICodexTicketModelsContext(ctx context.Context) []string {
	models := s.openAICodexTicketConfig().Models
	if s != nil && s.settingService != nil {
		return s.settingService.GetOpenAICodexTicketModels(ctx, models)
	}
	return models
}
...
func (s *OpenAIGatewayService) openAICodexTicketGatedModelContext(ctx context.Context, model string) bool {
	...
	if model == "" || !s.openAICodexTicketEnabledContext(ctx) {
		return false
	}
	for _, item := range s.openAICodexTicketModelsContext(ctx) {
...
func OpenAICodexTicketStatuses(account *Account, cfg config.OpenAICodexTicketConfig, now time.Time) []OpenAICodexTicketStatus {
	...
	for _, model := range models {
		...
		rev := parseCodexTicketRevocation(account, model)
		if rev.Revoked {
			status.RevocationReason = rev.Reason
			status.RevokedAt = &rev.At
		}
		...
		if !rev.Revoked && ticket.valid(now, targetLen) && ticket.Version == rev.Version {
			status.Ready = true
...
func (s *OpenAIGatewayService) lookupOpenAICodexTicket(account *Account, model string) *openAICodexTicket {
	...
	configuredLength := openAICodexTicketDefaultLength
	if s != nil {
		configuredLength = s.openAICodexTicketConfig().TargetLength
	}
	targetLen := openAICodexTicketTargetLength(account, configuredLength)
	now := time.Now()
	...
	if extra != nil && extra.Version != state.Version {
		extra = nil
	}
	if mem != nil && mem.Version != state.Version {
		mem = nil
	}
...
func (s *OpenAIGatewayService) storeOpenAICodexTicket(ctx context.Context, account *Account, ticket *openAICodexTicket, expected ...uint64) bool {
	...
	state.Lock()
	defer state.Unlock()
	// Deselection discards probe results without deleting existing tickets.
	if len(expected) > 0 && !s.openAICodexTicketGatedModelContext(ctx, model) {
		return false
	}
	if state.dirty || (len(expected) > 0 && expected[0] != state.Version) || (state.Revoked && len(expected) == 0) {
		return false
	}
	ticket.Version = state.Version
	next := openAICodexTicketRevocation{Version: state.Version}
	if !s.persistCodexTicketState(ctx, state, ticket, next) {
		return false
	}
	if state.Revoked {
...
func (s *OpenAIGatewayService) applyOpenAICodexTicket(ctx context.Context, account *Account, model string, h http.Header, bindings ...*openAIWSTicketBinding) error {
	if s == nil || h == nil || !isOpenAICodexTicketAccount(account) || !s.openAICodexTicketEnabledContext(ctx) {
		return nil
	}
	model = normalizeOpenAICodexTicketModel(model)
	if model == "" || !s.openAICodexTicketGatedModelContext(ctx, model) {
		return nil
	}
	...
	if ticket.valid(time.Now(), openAICodexTicketTargetLength(account, cfg.TargetLength)) {
		...
		for _, binding := range bindings {
			if binding != nil {
				*binding = openAIWSTicketBinding{model: model, expiresAt: ticket.ExpiresAt, version: ticket.Version}
			}
		}
		return nil
	}
...
func (s *OpenAIGatewayService) fireOpenAICodexTicketProbe(ctx context.Context, account *Account, token, model, proxyURL string, attemptTimeout time.Duration, attemptOut ...*uint64) (state string, status int, err error) {
	...
	if !s.openAICodexTicketGatedModelContext(ctx, model) {
		return "", 0, errOpenAICodexTicketModelDisabled
	}
	started := time.Now()
	...
	defer func() {
		...
		case len(state) != openAICodexTicketTargetLength(account, s.openAICodexTicketConfig().TargetLength):
	...
	// Recovery is driven solely by the ticket header; do not drain the stream.
	defer func() {
		if resp.Body != nil {
			_ = resp.Body.Close()
		}
...
func (s *OpenAIGatewayService) StartOpenAICodexTicketHarvester() {
	...
	logger.L().Info("openai_codex_ticket harvester started",
		...
		zap.Strings("default_models", s.openAICodexTicketConfig().Models),
	)
}
...
func (s *OpenAIGatewayService) refreshOpenAICodexTickets(ctx context.Context) {
	s.retryCodexTicketRevocations(ctx)
	...
	models := s.openAICodexTicketModelsContext(ctx)
	...
	for i := range accounts {
		...
		for _, model := range models {
			...
			if t := s.lookupOpenAICodexTicket(&account, model); t.valid(now, openAICodexTicketTargetLength(&account, cfg.TargetLength)) && !t.needsRefresh(now, refreshBefore) {
				continue
			}
			acc := account
...
// probeOnceOpenAICodexTicket 走打票代理打一发。命中合格门票（HTTP 200、长度==账号目标、
...
func (s *OpenAIGatewayService) probeOnceOpenAICodexTicket(ctx context.Context, account *Account, model string) {
	if s == nil || !isOpenAICodexTicketAccount(account) || ctx.Err() != nil || !s.openAICodexTicketGatedModelContext(ctx, model) {
		return
	}
	cfg := s.openAICodexTicketConfig()
	...
	_, _, _ = s.openaiCodexTicketFlight.Do(key, func() (any, error) {
		...
		if errors.Is(perr, errOpenAICodexTicketHarvestPaused) || errors.Is(perr, errOpenAICodexTicketModelDisabled) {
			return nil, nil
		}
		if perr != nil {
			if attempt == 0 {
		...
		if status != http.StatusOK || state == "" || len(state) != openAICodexTicketTargetLength(account, cfg.TargetLength) || !strings.HasPrefix(state, openAICodexTicketStatePrefix) {
			logger.L().Info("openai_codex_ticket probe miss",
				zap.Int64("account_id", account.ID), zap.String("model", model),
...
func IsOpenAICodexTicketExtraKey(key string) bool {
	return strings.HasPrefix(key, openAICodexTicketExtraKeyPrefix) || strings.HasPrefix(key, "codex_turn_ticket_revocation:")
}
